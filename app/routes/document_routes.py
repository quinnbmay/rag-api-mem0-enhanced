...
class MemoryRequest(BaseModel):
    content: str
    user_id: Optional[str] = MEM0_USER_ID
...
class HybridMemorySystem:
    def __init__(self):
        ...
        if DRAGONFLY_URL:
            try:
                self.dragonfly = redis.Redis.from_url(DRAGONFLY_URL)
                self.dragonfly.ping()  # Test connection
                logger.info("DragonflyDB connected successfully")
    ...
    async def search_mem0(self, query: str, user_id: str) -> Dict[str, Any]:
        """Search Mem0 personal memories"""
        if not self.mem0_headers:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {"error": f"Mem0 API error: {response.status_code}"}
        except Exception as e:
            logger.error(f"Mem0 search error: {e}")
            return {"error": str(e)}
    
    async def add_mem0_memory(self, content: str, user_id: str) -> Dict[str, Any]:
        """Add memory to Mem0"""
        if not self.mem0_headers:
            return {"error": "Mem0 not configured"}
        ...
        try:
            response = requests.post(
                ...
                headers=self.mem0_headers,
                json={"messages": [{"content": content}], "user_id": user_id},
                timeout=10
            )
            if response.status_code == 200:
                # Invalidate cache
                if self.dragonfly:
                    try:
                        cache_keys = self.dragonfly.keys("hybrid_search:*")
                        if cache_keys:
                            self.dragonfly.delete(*cache_keys)
                    except Exception as e:
    ...
    def get_cache(self, key: str) -> Optional[Dict[str, Any]]:
        ...
        if not self.dragonfly:
            return None
        try:
            cached = self.dragonfly.get(key)
            if cached:
    ...
    def set_cache(self, key: str, value: Dict[str, Any], expire: int = 3600):
        """Set cached result in DragonflyDB"""
        if not self.dragonfly:
            return
        try:
            self.dragonfly.setex(key, expire, json.dumps(value))
        except Exception as e:
...
async def add_memory(request: MemoryRequest):
    ...
    try:
        result = await hybrid_memory.add_mem0_memory(request.content, request.user_id)
        if "error" in result:
...
async def search_memory(request: MemorySearchRequest):
    ...
    try:
        result = await hybrid_memory.search_mem0(request.query, request.user_id)
        if "error" in result:
            raise HTTPException(status_code=400, detail=result["error"])
        return result
    except Exception as e:
...
async def hybrid_search(request: HybridSearchRequest):
    ...
    1. Check DragonflyDB cache (hot)
    2. Search Mem0 personal memories (warm)
    3. Query RAG API for documents (cold)
    """
    try:
        cache_key = f"hybrid_search:{hash(request.query)}:{request.user_id}:{request.file_id}"
        
        # 1. Hot tier - Check cache
        if request.use_cache:
            cached_result = hybrid_memory.get_cache(cache_key)
            if cached_result:
        ...
        response = {
            ...
            "timestamp": str(hash(request.query))[:19],  # Simple timestamp placeholder
            "hot_tier": None,
            "warm_tier": None,
            "cold_tier": None
        }
        
        # 2. Warm tier - Mem0 search
        mem0_result = await hybrid_memory.search_mem0(request.query, request.user_id)
        if "error" not in mem0_result:
            response["warm_tier"] = {
                "source": "mem0",
                "results": mem0_result.get("results", []),
                "count": len(mem0_result.get("results", []))
            }
        
        # 3. Cold tier - RAG document search
        try:
            embedding = get_cached_query_embedding(request.query)
            ...
            if isinstance(vector_store, AsyncPgVector):
                if request.file_id:
                    documents = await vector_store.asimilarity_search_with_score_by_vector(
                        embedding,
                        k=request.k,
                        filter={"file_id": request.file_id}
                    )
                else:
                    documents = await vector_store.asimilarity_search_with_score_by_vector(
                        embedding,
                ...
                if request.file_id:
                    documents = vector_store.similarity_search_with_score_by_vector(
                        embedding, k=request.k, filter={"file_id": request.file_id}
                    )
                else:
                    documents = vector_store.similarity_search_with_score_by_vector(
                        embedding, k=request.k
                    )
        ...
        # Cache the result
        if request.use_cache:
            hybrid_memory.set_cache(cache_key, response, expire=3600)
            logger.info(f"Cached hybrid search result for: {request.query}")
        
        # Mark as cache miss
        response["hot_tier"] = "cache_miss"
...
async def memory_system_status():
    ...
    if hybrid_memory.dragonfly:
        try:
            hybrid_memory.dragonfly.ping()
            status["dragonfly"]["connected"] = True
        except:
            status["dragonfly"]["connected"] = False
